// src/constants.ts
const VISION_PLUGIN_NAMESPACE = 'vision-plugin';
// src/index.ts
import z from '@deepseek-ai/schemastery';
import { settingsNamespace } from '@deepseek-ai/dsh-settings';
import { symbols } from '@deepseek-ai/cordis';
const VisionPluginSettingsSchema = z.object({
...
const DEFAULT_TRANSCRIBE_PROMPT = 'Transcribe and describe this image faithfully. Include all visible text verbatim (OCR). ' + 'Then briefly describe any other relevant content such as diagrams, charts, UI elements, people, or the scene. ' + 'Reply with only the transcription and description, without preamble.';
...
function collectImageRefs(content, out) {
  for (const block of content) {
    ...
    if (block.type === 'tool-result' && typeof block.content !== 'undefined') {
...
function mapBlocksReplacingImages(content, nextTranscription) {
  ...
  for (const block of content) {
    if (block.type === 'image') {
      ...
      changed = true;
    } else if (block.type === 'tool-result' && typeof block.content !== 'undefined') {
      const inner = mapBlocksReplacingImages(block.content, nextTranscription);
      if (inner === void 0) {
        result.push(block);
      } else {
...
function installImageGuardBypass(ctx, scope, llm) {
  ...
  const apply = (enabled) => { enabled ? activate() : deactivate(); };
...
function installImageTranscription(ctx, scope, llm) {
  ...
  const transcribingStream = async function* (options, prepared) {
    ...
    try {
      ...
      yield {
        type: 'finish',
        reason: { kind: 'error', failure: { message: `vision-plugin: image transcription failed: ${errorMessage(error)}`, code: 'VISION_TRANSCRIBE_FAILED' } }
      };
    }
  };
  ctx.effect(() => {
...
function exposeSettingsNamespace(ctx) {
...
function apply(ctx) {
  ctx.inject(['settings', 'llm'], (both) => {
    const scope = both.settings.register(settingsNamespace(VISION_PLUGIN_NAMESPACE), VisionPluginSettingsSchema);
    exposeSettingsNamespace(both);
    installImageGuardBypass(both, scope, both.llm);
    installImageTranscription(both, scope, both.llm);
  });
}
export { VISION_PLUGIN_NAMESPACE, apply };
